...
console.log("Using Loop Even: " + newArrLoop); // Using Loop Even: 2,4

// 2.  Using filter
const ans = arr.filter((value) => {
    return value % 2 === 0;
});
console.log("Using Filter Even: " + ans); // Using Filter Even: 2,4
...
const name = ["harkirat", "raman", "pratham"]

const answer = name.filter((n) => {
    if (n.startsWith('h')) {
        return true
    } else {
        return false
